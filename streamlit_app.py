...
def update_weather():
    ...
    if datetime.datetime.utcnow() > weather_cache["expires"]:
        response_current = requests.get(f'https://api.met.no/weatherapi/locationforecast/2.0/complete?lat={lat}&lon={lng}', headers=headers)
        ...
        if response_current.status_code == 200:
            ...
            weather.markdown(f'<div id="weather">{temp}°C<br>UV {uv}<i class="big-icon wi {weathercode}"></i></div>', unsafe_allow_html=True)
        else:
...
def work_schedule():
    ...
    for event_summary in work:
        html_content += f'<div class="event"><span class="time">{event_summary}</span><i class="fa-solid fa-laptop-code"></div>'
    html_content += '</i></div>'
...
def render_swim_donut():
    ...
    svg = generate_donut_chart_svg_from_counter(counter)
    b64 = base64.b64encode(svg.encode('utf-8')).decode("utf-8")
    html = f'<div id="swims"><span class="count">{swim_days}</span><span>swim<br>days</span><img src="data:image/svg+xml;base64,{b64}" /></div>'
    st.write(html, unsafe_allow_html=True)
